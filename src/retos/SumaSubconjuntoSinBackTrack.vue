...
const backTrack = async (numeros, objetivo) => {
  ...
  console.log(numeros, objetivo);
  const result = [];
  // Esto es muy lento y no es la mejor forma de hacerlo, pero es una forma de hacerlo.

  for (let i = 0; i < numeros.length; i++) {
    let suma = 0;
    let sublista = [];
    // recorremos los todos los números y los sumamos
    for (let j = i; j < numeros.length; j++) {
      suma = +numeros[j];
      if (sublista.reduce((acc, val) => acc + val, 0) === objetivo) {
